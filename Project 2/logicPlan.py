...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    sentence = conjoin(premise, ~conclusion)
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # Get all pairs of literals in the list
    pairs = list(itertools.combinations(literals, 2))
    # Construct the CNF expression (A | ~B) & (~A | B) for each pair of literals
    clauses = [disjoin(~A, ~B) for A, B in pairs]
    # Return the conjunction of all clauses
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    # Calculate the at most one expression to be true and add it to the conjunction
    at_most_one = atMostOne(literals)
    # Combine the at least one and at most one expressions using an AND operator
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    #  所有possible_causes中有true的

    return PropSymbolExpr(pacman_str, x, y, time=now) % disjoin([possible_causes_sent])
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for coord in non_outer_wall_coords:
        possible_coord_expr.append(PropSymbolExpr(pacman_str, coord[0], coord[1], time=t))
    pacphysics_sentences.append(exactlyOne(possible_coord_expr))

    # Pacman takes exactly one action at timestep t.
    possible_action_expr = []
    for action in DIRECTIONS:
        possible_action_expr.append(PropSymbolExpr(action, time=t))
    pacphysics_sentences.append(exactlyOne(possible_action_expr))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    """
        Add to KB: pacphysics_axioms(...) with the appropriate timesteps. There is no sensorModel because we know 
        everything about the world. Where needed, use allLegalSuccessorAxioms for transitions since this is for regular 
        Pacman transition rules. 
        """
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        location_sentences = [] #add the location of every step to KB
        ...
        direction_sentences = [] #add the direction of every step to KB
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        goal_sentences = []
        for coord in food:
            goal_sentences.append(~PropSymbolExpr(food_str, coord[0], coord[1], time=t))

        model = findModel(conjoin(goal_sentences) & conjoin(KB))
        if model:
            # print(extractActionSequence(model, actions))
            return extractActionSequence(model, actions)
