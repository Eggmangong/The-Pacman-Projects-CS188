...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in variables:
        ...
        if i in [PAC, GHOST0, GHOST1]:
            for a in range(X_RANGE):
                for b in range(Y_RANGE):
                    variableDomainsDict[i].append((a, b))
        else:
            for j in range(MAX_NOISE + X_RANGE + Y_RANGE - 1):
                variableDomainsDict[i].append(j)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            currentFactors, joinFactor = joinFactorsByVariable(currentFactors, variable)
            if len(joinFactor.unconditionedVariables()) >= 2:
                eliminated = eliminate(joinFactor, variable)
                currentFactors.append(eliminated)

        jointCurrentFactor = joinFactors(currentFactors)
        normalized = normalize(jointCurrentFactor)
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        realDistance = busters.manhattanDistance(pacmanPosition, ghostPosition)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            ...
            self.beliefs[position] *= self.getObservationProb(noisyDis, pacmanPos, ghostPos, jailPos)
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos in newPosDist.keys():
                new[newPos] += newPosDist[newPos] * p
            # else:
            #     new[newPos] = newPosDist[newPos] * p
        self.beliefs = new
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            particle = i % len(self.legalPositions)
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for partical in self.particles:
            weighted[partical] += self.getObservationProb(observation, pacPos, partical, jailPos)
        if weighted.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [weighted.sample() for each in range(self.numParticles)]
